 ...
 * 
 *//**
	 * 1、资源服务器配置类： 当用户请求某个微服务资源时，首先要进行认证与授权，通过后再才可访问到对应资源。
	 * 
	 ...
	 * 务器，资源服务器会通过事先约定好的算法进行解析出来，然后直接对JWT令牌校验，不需要每次远程请求认证服 务器完成授权。
	 */
/*
 * @Configuration
 * 
 ...
 * 
 * @EnableGlobalMethodSecurity(prePostEnabled = true) // 开启方法级别权限控制 public class
 ...
 * throws Exception { resources.resourceId(RESOURCE_ID).tokenStore(tokenStore);
 * }
 * 
 *//**
	 * 7、资源服务器通过该方法指定授权规则。
	 *//*
		 ...
		 * http.sessionManagement() // SpringSecurity不会使用也不会创建HttpSession实例
		 * .sessionCreationPolicy(SessionCreationPolicy.STATELESS) .and()
		 * .authorizeRequests() // 授权规则配置 //
		 * .antMatchers("/product/*").hasAuthority("product") // 所有请求，都需要有all范围（scope）
		 * .antMatchers("/**").access("#oauth2.hasScope('all')") // 等价于上面 //
